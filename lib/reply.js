...
function replyArticlesToClick(articleCount,msg){
    ...
    var article_item_1 = "<item>" +
        "<Title>番茄炒蛋</Title>" +
        "<Description>炒：炒是使用最广泛的一种烹调方法，用油量的高低决定热量高低，从而影响其在减肥期间的适宜程度，减肥期间推荐清淡少油的清炒菜肴。</Description>" +
        "<PicUrl>"+tomato_picUrl_big+"</PicUrl>" +//图片链接
        "<Url>"+tomato_url+"</Url>" +//点击图文跳转链接
        "</item>";
    var articles="";
    articles += article_item_1;
    var replyTmpl = "<xml>" +
        "<ToUserName><![CDATA[{toUser}]]></ToUserName> " +
        "<FromUserName><![CDATA[{fromUser}]]></FromUserName> " +
        "<CreateTime><![CDATA[{time}]]></CreateTime> " +
        ...
        "<Articles><![CDATA[{articles}]]></Articles> " +
        "</xml>";
    var tmpl = require("tmpl");
    return  tmpl(replyTmpl,{
        toUser : msg.xml.FromUserName[0],
